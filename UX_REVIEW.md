...
Verified live in Chrome after the fix: the icon and "for shopping" label are gone, no residual gutter, ordinary page padding restored.

**Second gap found the same walkthrough, on product detail pages:** the inline "Ask Alexa or search reviews and Q&A" widget (`#dpx-rex-nice-widget-container`, with descendant ids like `dpx-rex-nile-search-text-input`) is a third, separate Rufus surface — distinct from both the nav trigger and the docked panel above — and wasn't covered by any existing selector either. Added `#dpx-rex-nice-widget-container` and the `[id^="dpx-rex-"]` prefix to `ASSISTANT_SELECTORS` so the whole widget (input, submit button, suggested-question pills, answer area) is hidden as one unit. Verified live: hidden with no leftover blank space (it's inline content, not a docked/padded panel, so there was no gutter risk here).

Not yet re-verified: the same checks on other Amazon locales, on dynamically-loaded/lazy Rufus mounts, or after Amazon changes this markup again — selector drift on a live third-party site is an ongoing risk, not a one-time fix, and other Rufus surfaces (e.g. review-page or search-result inline widgets) have not been walked yet. Continue with the rest of Section B of LOCAL_TESTING.md (both chat arms, other categories, resize/zoom) before treating classic as fully verified.
